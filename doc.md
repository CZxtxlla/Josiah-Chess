...
### Quiescence
Quiescence essentially extends the search, once negamax reaches depth 0, until a quiet position is reached. It ensures that only quiet positions are ever evaluated, mitigating the horizon effect. It takes a bit of extra computation time but is well worth it for this reason. Implementing it is as simple as implementing negamax but instead of having a base case of depth = 0, the base case is the position is quiet.
